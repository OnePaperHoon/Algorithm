...
### 입력 
 ...
 <p>총 `다섯줄`의 입력이 주어진다. 각 `줄에는 최소 1개, 최대 15개의 글자`들이 빈칸 없이 `연속`으로 주어진다. 주어지는 글자는 영어 대문자 ‘A’부터 ‘Z’, 영어 소문자 ‘a’부터 ‘z’, 숫자 ‘0’부터 ‘9’ 중 하나이다. 각 줄의 시작과 마지막에 빈칸은 없다.</p>
